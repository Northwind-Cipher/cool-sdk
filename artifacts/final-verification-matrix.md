# CooL x Phala Cloud - Verification Matrix (Run 2, superseded by closure-verification-matrix.md)

This run used a witness key generated by the validating script and a consistency check outside the receipt verifier. The closure run replaces both: a witness in its own CVM with an attested sealed key, and consistency verified with a public verifier plus an independent implementation.
